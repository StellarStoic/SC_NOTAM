...
if not notam_data_elements:
    ...
    notams_by_day = {today + timedelta(days=i): [] for i in range(3)}
    ...
    for notam_data in notam_data_elements:
        ...
        if start_date is not None and end_date is not None:
            # Check if the NOTAM is within the next 7 days
            for day, notams in notams_by_day.items():
                if today <= day <= today + timedelta(days=3):
                    if start_date.date() <= day <= end_date.date():
                        notams.append(notam_data_tuple)

    # Store the NOTAMs for each day in a separate dictionary
    notams_data = {}
    for day, notams in notams_by_day.items():
        notams_data[day] = []
        if notams:
            for notam_data_tuple in notams:
                notams_data[day].append(notam_data_tuple)

    # Manipulate the NOTAMs data as needed
    # Example: Print the NOTAMs for each day with feet to meters conversion for F) and G) data
    for day, notams in notams_data.items():
        print("\n----------------------------------------")
        date_label = day.strftime('%A %d.%m.%Y')
        print(date_label)
        if notams:
            for notam_data_tuple in notams:
                print("----------------------------------------\n")
